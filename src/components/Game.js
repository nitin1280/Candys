...
const Game = () => {
  ...
  // Checks for win when timer stops.
  useEffect(() => {
    // Inside the useEffect where you determine the game result
    if (timeLeft === 0) {
      if (scoreDisplay >= 50) {
  ...
  return (
    <div className="app">
      <ScoreBoard
        ...
        gamesWon={gamesWon} // Pass the number of games won to ScoreBoard
        gamesLost={gamesLost} // Pass the number of games lost to ScoreBoard
      />
      ...
      <div className="game">
        {currentColorArrangement.map((candyColor, index) => (
          <img
            ...
            onDrop={dragDrop}
            onDragEnd={dragEnd}
            onClick={() => handleCandyClick(index)} // Handle candy click
          />
        ))}
      </div>
    </div>
